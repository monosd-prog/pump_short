...
def run_audit(
    ...
    for outcomes_path, date_val, strategy in files:
        ...
        for row in rows:
            ...
            entry = float(trade.get("entry_price") or row.get("entry_price") or row.get("entry") or 0)
            tp = float(trade.get("tp_price") or row.get("tp_price") or row.get("tp") or 0)
            sl = float(trade.get("sl_price") or row.get("sl_price") or row.get("sl") or 0)
            opened_ts = (trade.get("entry_time_utc") or row.get("opened_ts") or "").strip()
